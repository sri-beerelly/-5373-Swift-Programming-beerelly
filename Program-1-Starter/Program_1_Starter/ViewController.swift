...
class ViewController: UIViewController {
                ...
                if(error != nil) {
                   ...
                   /* for item in json{
                        for (key , value) in item.1{
                            //println("\(value)")
                            if key == "colorName" {
                            self.colorNames.append(String(value.string!))
                                //println("\(value)")
    ...
    func matchColorLogic() -> Bool{
        ...
        var randColor = self.myColors.fetchRandomColor()
        var randColor2 = self.myColors.fetchRandomColor()
        var randColor3 = self.myColors.fetchRandomColor()
        
        btnAnswer1OUTLET.titleLabel?.numberOfLines=2
        btnAnswer2OUTLET.titleLabel?.numberOfLines=2
        btnAnswer3OUTLET.titleLabel?.numberOfLines=2
        
        var color = colorNumber
        var prev = ((colorNumber - 1) + colorNames.count) % colorNames.count
        var next = (colorNumber + 1) % colorNames.count
        
        btnAnswer1OUTLET.setTitle(randColor.colorName, forState: UIControlState.Normal)
        btnAnswer2OUTLET.setTitle(randColor2.colorName, forState: UIControlState.Normal)
        btnAnswer3OUTLET.setTitle(randColor3.colorName, forState: UIControlState.Normal)
        
        if(buttonAnswer==0)
        {
            changeColor(randColor.hexValue)
            button1Correct = true;
            button2Correct=false;
            button3Correct=false;
        }
        if(buttonAnswer==1){
            changeColor(randColor2.hexValue)
            button2Correct = true;
            button1Correct=false;
            button3Correct=false;
        }
        if(buttonAnswer==2)
        {
            changeColor(randColor3.hexValue)
            button3Correct = true;
            button2Correct=false;
            button1Correct=false;
        }
        ...
        if(buttonAnswer == 1){
            btnAnswer1OUTLET.setTitle(randColor3.colorName, forState: UIControlState.Normal)
            btnAnswer2OUTLET.setTitle(randColor2.colorName, forState: UIControlState.Normal)
            btnAnswer3OUTLET.setTitle(randColor.colorName, forState: UIControlState.Normal)
            button2Correct = true
        }
        
        if(buttonAnswer == 2){
            btnAnswer1OUTLET.setTitle(randColor2.colorName, forState: UIControlState.Normal)
            btnAnswer2OUTLET.setTitle(randColor.colorName, forState: UIControlState.Normal)
            btnAnswer3OUTLET.setTitle(randColor3.colorName, forState: UIControlState.Normal)
            button3Correct = true
        }
        */
    ...
    func changeColor(strVal:String) -> Bool{
        
        lblChangeColor.backgroundColor=myColors.hexStringToUIColor(strVal)
        /*if(colorNumber == 0){
